...
(defn main-handler-enter
  [{:keys [request] :as ctx} registry dispatch-map handler-component router]
...
(defn main-handler
  ...
  [registry dispatch-map handler-component]
  (let [router (dispatch-map->bidi dispatch-map)]
    {:name ::main-handler
     :enter (fn [ctx]
              (main-handler-enter ctx registry dispatch-map handler-component router))}))
